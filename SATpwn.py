...
class SATpwn(plugins.Plugin):
    ...
    def __init__(self):
        ...
        logging.info("[SATpwn] Plugin initializing with flat TOML key support...")
        self._load_config()

    def _load_config(self):
        """Load configuration from flat TOML keys only."""
        try:
            ...
            # Load plugin enable/disable setting
            ...
            # If plugin is disabled, skip loading other configs
            if not self.plugin_enabled:
                logging.info("[SATpwn] Plugin disabled via config - stopping initialization")
                return
    ...
    def on_loaded(self):
        ...
        if not self.plugin_enabled:
            logging.info("[SATpwn] Plugin disabled via config - plugin will not activate")
            return
    ...
    def on_ready(self, agent):
        ...
        enabled_status = "ENABLED" if self.plugin_enabled else "DISABLED"
        logging.info(f"[SATpwn] Plugin ready - Status: {enabled_status}, Mode: {self.mode} ({exclusive_status})")
    ...
    def on_webhook(self, path, request):
        ...
        if path == '/' or not path:
            ...
            # Plugin status
            plugin_status = ""
            enabled_color = "#4CAF50" if self.plugin_enabled else "#f44336"
            enabled_text = "ENABLED" if self.plugin_enabled else "DISABLED"
            ...
            if self.disable_defaults:
                ...
                plugin_status = f"""
                <p style="color: {enabled_color};"><b>Plugin Status: {enabled_text}</b></p>
            ...
            <body>
                ...
                <div class="container">
                    ...
                    <div class="card">
                        ...
                        <p><code>main.home_whitelist = "{','.join(self.home_whitelist)}"</code></p>
                    </div>
                    <div class="card">
